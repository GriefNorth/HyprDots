...
_download_pre_release() {
  ...
  latest_tag=$(curl -s https://api.github.com/repos/GriefNorth/HyprDots/releases |
  ...
  if git clone --branch "$latest_tag" --depth 1 https://github.com/GriefNorth/HyprDots.git "$DOWNLOAD_DIR"; then
...
_download_rolling_release() {
  ...
  if git clone --depth 1 https://github.com/GriefNorth/HyprDots.git "$DOWNLOAD_DIR"; then
